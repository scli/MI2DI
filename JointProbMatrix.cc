...
//!!!!!!!!!!!!!!!!!!!=============================================!!!!!!!!!!!!!
//count how many pairs, for each pair of clusters. 
//
//The following method is for Sander: all the pairs 
//of columns have the same weight
void
JointProbMatrix::countSanderNumPairs()
{
   ...
   //First, compute replication weight
   for(int i=0; i<num_replicates; i++)
   {
      ...
      {
         weight+=mData->similarity(mRawData[j], mRawData[j]);
      }//for
   ...
   for(int i=0; i<num_replicates; i++)
   {
      for(int pos1=0; pos1<size; pos1++)
      {
         ...
         mNumSingular[pos1][index1]+=1/mReplicateWeight[i];
         ...
         for(int pos2=0; pos2<size; pos2++)
         {
           ...
           mNumPairMatrix[off1+index1][off2+index2]+=1/mReplicateWeight[i];
         }//for
      }//for
